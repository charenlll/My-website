...
- 方向：产品设计、数据分析、AI应用开发、海外产品运营与空间研究。
- 实习：Construct｜产品与运营实习生，2026.06—2026.09。负责游戏活动PRD、原型、数值设计、开发对接、验收和资源配置全流程；覆盖菲律宾、缅甸、印尼及华语4个市场，约1万人参与，带动充值与金币消耗增长。独立开发素材工作台，在部门产品与运营团队落地，按素材类型制作效率提升2—5倍。对接华语区当地运营，提供高付费用户与代理招募方案。
- 实习：华为｜大模型算法，2026.04—2026.05。参与基于激光点云与Qwen视觉语言模型的道路要素提取及模型训练；采用SFT冷启动与GRPO优化，设计涵盖格式约束、几何特征和召回率的复合奖励函数，改善识别平滑度与稳定性。
- 实习：美团｜数据分析，2025.07—2025.08。处理日均120万+共享单车订单，结合职住分布定位地铁接驳场景的淤积与缺车点，参与“运维前置+热点区域增投”方案并推动试点落地。
- 实习：武汉市一卡通支付有限公司｜项目助理，2025.02—2025.04。匹配超千万条公交刷卡记录与GPS链路数据，构建乘客出行链；对接运营与技术部门梳理需求、统一统计口径，输出客流与换乘效率报表。
- 实习：合肥市规划设计研究院｜规划助理，2023.07—2023.09。参与产业园规划，量化区位交通与产业关联度，构建产业适宜性评价模型，支撑功能布局及配套设施选址。
- 项目职责：Wriothesley桌面端、AI功能、云同步与展示站全部独立完成，项目起止日期未提供。使用Electron、JavaScript、DeepSeek API及Supabase；实现结构化任务指令、原生OCR、4类数据云同步、行级访问控制及本地原子写入与备份恢复。
- 指标口径：约1万人为游戏活动参与规模，未确认单场口径；充值与金币消耗增长未提供可靠比例，不得编造。音乐盛典交接时PRD完成约40%，不能称已上线或取得业绩。
...
- 软件项目二：Wriothesley。基于 Electron 与 DeepSeek API 的桌面助手，常驻桌面，支持桌面角色（透明置顶、自由拖拽，并记住上次关闭位置）、AI 聊天（角色设定、历史记忆、上下文对话）、计划与周报（聊天写入计划，完成后整理周报）、截图 OCR（区域截图、保存、复制、文字识别与翻译）、便签与短文本、设置与装扮（管理 API、快捷键、主题、透明度和角色外观）。
...
addMessage(
  "你好，我是陈浩文个人作品集的 AI 介绍助手。我会根据本站公开资料回答有关他的实习经历、项目实践和能力结构的问题。请选择一个推荐问题，或直接输入你想了解的内容。",
  "bot",
);
...
async function sendMessage() {
  ...
  try {
    ...
    if (!response.ok) throw new Error("AI服务暂不可用");
